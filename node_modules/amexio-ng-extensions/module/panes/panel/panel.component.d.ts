import { AfterViewInit, EventEmitter, OnChanges, OnDestroy, OnInit, Renderer2, SimpleChanges } from '@angular/core';
import { LifeCycleBaseComponent } from '../../base/lifecycle.base.component';
export declare class AmexioPanelComponent extends LifeCycleBaseComponent implements AfterViewInit, OnInit, OnChanges, OnDestroy {
    private renderer;
    document: any;
    title: any;
    header: boolean;
    expanded: boolean;
    border: boolean;
    collapsible: boolean;
    height: number;
    contextmenu: any[];
    color: string;
    bgColor: string;
    contentAlign: string;
    parentRef: any;
    fit: boolean;
    onClick: EventEmitter<any>;
    nodeRightClick: any;
    rightClick: any;
    iconclassKey: string;
    panelWidth: any;
    flag: boolean;
    gradientFlag: boolean;
    posixUp: boolean;
    rightClickNodeData: any;
    contextStyle: any;
    mouseLocation: {
        left: number;
        top: number;
    };
    private faFaIconUPCss;
    private faFaIconDownCss;
    themeCss: any;
    amexioComponentId: string;
    panelstyle: any;
    componentId: any;
    fullscreenMax: boolean;
    globalClickListenFunc: () => void;
    constructor(renderer: Renderer2, document: any);
    ngOnInit(): number;
    ngAfterViewInit(): void;
    ngOnChanges(changes: SimpleChanges): void;
    onTabClick(btn: any): void;
    togglePanel(expand: any): void;
    rightClickDataEmit(Data: any): void;
    private updatestyle;
    getContextMenu(): void;
    getListPosition(elementRef: any): boolean;
    loadContextMenu(rightClickData: any): void;
    addListner(): void;
    removeListner(): void;
    ngOnDestroy(): void;
    setColorPalette(themeClass: any): void;
    changeHeaderColor(): void;
}
