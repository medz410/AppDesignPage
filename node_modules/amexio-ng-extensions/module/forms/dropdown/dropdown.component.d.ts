/**
 * Created by pratik on 1/12/17.
 */
import { ChangeDetectorRef, ElementRef, OnInit, Renderer2, TemplateRef } from '@angular/core';
import { ControlValueAccessor, FormControl, Validators } from '@angular/forms';
import { EventBaseComponent } from '../../base/event.base.component';
import { CommonDataService } from '../../services/data/common.data.service';
import { DisplayFieldService } from '../../services/data/display.field.service';
export declare class AmexioDropDownComponent extends EventBaseComponent<any> implements OnInit, ControlValueAccessor, Validators {
    dataService: CommonDataService;
    private displayFieldService;
    element: ElementRef;
    renderer: Renderer2;
    fieldlabel: string;
    allowblank: boolean;
    _data: any;
    componentLoaded: boolean;
    data: any;
    datareader: string;
    httpmethod: string;
    httpurl: string;
    displayfield: string;
    valuefield: string;
    search: boolean;
    readonly: boolean;
    multiselect: boolean;
    buttonLabel: string;
    dropdownitems: ElementRef;
    displayValue: string;
    filteredOptions: any[];
    selectAllFlag: boolean;
    onBlur: any;
    input: any;
    focus: any;
    onSingleSelect: any;
    onRecordSelect: any;
    onMultiSelect: any;
    onClick: any;
    showToolTip: boolean;
    placeholder: string;
    disabled: boolean;
    iconfeedback: boolean;
    fontstyle: string;
    fontfamily: string;
    fontsize: string;
    haslabel: boolean;
    enablepopover: boolean;
    enablesort: boolean;
    sort: string;
    enablecheckbox: boolean;
    maskloader1: boolean;
    helpInfoMsg: string;
    _errormsg: string;
    errormsg: string;
    bodyTemplate: TemplateRef<any>;
    posixUp: boolean;
    isValid: boolean;
    selectedindex: number;
    responseData: any;
    previousData: any;
    viewData: any;
    componentId: string;
    multiselectValues: any[];
    maskloader: boolean;
    activedescendant: string;
    key: string;
    isComponentValid: any;
    name: string;
    constructor(dataService: CommonDataService, displayFieldService: DisplayFieldService, element: ElementRef, renderer: Renderer2, _cd: ChangeDetectorRef);
    ngOnInit(): void;
    setData(httpResponse: any): void;
    setResponseData(responsedata: any): void;
    sortDataAscending(data: any): void;
    sortDataDescending(data: any): void;
    generateIndex(data: any): void;
    multiSelection(): void;
    bindData(): void;
    bindMultiselectModel(): void;
    splitData(): any;
    bindMultiSelectModelData(valueArray: any[]): void;
    setUserSelection(): void;
    enableChkbox(): void;
    emitItem(selectedItem: any): void;
    validateChkbox(): void;
    setFlags(): void;
    chkFlag(selectedItem: any): void;
    onItemSelect(selectedItem: any): void;
    checkboxMethod(selectedItem: any): void;
    setMultiSelectData(): void;
    modelCheck(modelValue: any, test: any): void;
    navigateKey(event: any): void;
    getDisplayText(): void;
    setMultiSelect(): any;
    onDropDownClick(event: any): void;
    generateScroll(index: any): void;
    focusToLast(event: any): void;
    closeOnEScape(event: any): void;
    onChange(event: string): void;
    onInput(input: any, event: any): void;
    onDropDownSearchKeyUp(event: any): void;
    navigateUsingKey(event: any): void;
    navigateFilterOptions(previndex: number): void;
    toNavigateFirstAndLastOption(): void;
    value: any;
    onblur(event: any): void;
    onFocus(elem: any, event: any): void;
    getListPosition(elementRef: any): boolean;
    writeValue(value: any): void;
    writeChangedValue(value: any): void;
    registerOnChange(fn: any): void;
    registerOnTouched(fn: any): void;
    onIconClick(): void;
    checkValidity(): boolean;
    validate(c: FormControl): {
        jsonParseError: {
            valid: boolean;
        };
    };
    selectAll(event: any): void;
    onSaveClick(event: any): void;
}
